...
def _optimize_seam_mask(front, back, w1, out_w, out_h, flow_unreliability=None):
    ...
    # Add strong edge penalty to avoid cutting through structure
    ...
    # Normalize edge magnitude to similar scale as diff
    edge_mag = edge_mag / (np.percentile(edge_mag[corridor_mask], 95) + 1e-6) * scale
    
    cost = diff + edge_mag * 5.0  # heavily penalize edges
