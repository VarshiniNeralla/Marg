...
def _regularize_strip_flow(fx, fy, guide_gray, x0, out_w, out_h, conf=None,
    ...
    if conf is not None:
        ...
        # Disabled "No-evidence shrink" to allow depth to extrapolate across textureless voids
        # par = par * ev

    return par * ux + perp_x, par * uy + perp_y, par
...
def _parallax_align_hemispheres(front, back, w1, out_w, out_h, clean1=None, clean2=None,
    ...
    for name, xc in strips:
        ...
        rx, ry, par_x = _regularize_strip_flow(
...
def _optimize_seam_mask(front, back, w1, out_w, out_h, flow_unreliability=None, flow_mag=None):
    ...
    for xc, side_lo, side_hi in (
        ...
        with open('par_values.txt', 'a') as f:
            f.write(f'path at 1000: {path[1000] + x0}\n')
            f.write(f'path at 1500: {path[1500] + x0}\n')
